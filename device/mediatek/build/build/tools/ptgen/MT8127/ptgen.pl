...
my $COMBO_NAND_KERNELH = "$CD_ALPS/$KERNEL_VERSION/arch/arm/mach-mt8127/$PROJECT/common/combo_nand.h";
...
my $EMMC_COMPO	= "$CD_ALPS/device/mediatek/build/build/tools/ptgen/MT8127/mbr_addr.pl" ;
...
my $COPY_PATH_KERNEL_PARTITION_H="$CD_ALPS/$KERNEL_VERSION/arch/arm/mach-mt8127/$PROJECT/common/";
...
my $COPY_PATH_PARTITION_DEFINE_H_NAME_K="$CD_ALPS/$KERNEL_VERSION/arch/arm/mach-mt8127/$PROJECT/common/";
my $COPY_PATH_PARTITION_DEFINE_C_NAME    = "$CD_ALPS/$KERNEL_VERSION/arch/arm/mach-mt8127/$PROJECT/common/";
...
my $COPY_PATH_PMT_H_NAME_K="$CD_ALPS/$KERNEL_VERSION/arch/arm/mach-mt8127/$PROJECT/common/";
...
if ($EMMC_SUPPORT eq "yes" || $MTK_NAND_UBIFS_SUPPORT eq "yes" || $MTK_NAND_MTK_FTL_SUPPORT eq "yes"){
	&GenPartSizeFile ();
}
				...
				if($MTK_NAND_MTK_FTL_SUPPORT eq "yes") {
					if(($PARTITION_FIELD[$index] eq "USRDATA") && ($temp == 0)){
						...
						if ($PARTITION_FIELD[$index] eq "CACHE") {
							...
							$temp = int(($SIZE_FIELD_KB[$index] / ($PEB/1024) - 20) * ($LEB/1024/1024)); #MB
							print "[Bean]system_size_ftl=$temp\n";
							#System Reserved UBI more than 20 for FTL ext4
						}
					}
					if(exists($PSalias{$PARTITION_FIELD[$index]})){
...
sub load_partition_info {
	...
	else
	{
		...
		my $SearchFile = 'partition_table_MT8127.xls'; #Search File Name
		...
		{
		#search folder list+{project}/partition_table_MT8127.xls existence
		...
		foreach $i (@arrayOfFirstLevelDirs)
		{
		#search folder list+{baseproject}/partition_table_MT8127.xls existence
...
sub do_copy_files_old {
	...
	if (-e "$SCAT_NAME_DIR/MT8127_Android_scatter.txt") {
		if(-e "$COPY_SCATTER_BR_FILES_PATH/MT8127_Android_scatter.txt"){
			`chmod 777 $COPY_SCATTER_BR_FILES_PATH/MT8127_Android_scatter.txt`;
		}
		`cp $SCAT_NAME_DIR/MT8127_Android_scatter.txt $COPY_SCATTER_BR_FILES_PATH`;
		print AUTO_CHECK_OUT_FILES "$COPY_SCATTER_BR_FILES_PATH/MT8127_Android_scatter.txt\n";
	}
...
sub do_copy_files {
	...
	if(($PTGEN_ENV eq "ALL") || ($PTGEN_ENV eq "PROJECT"))
	{
		...
		if (-e "$SCAT_NAME_DIR/MT8127_Android_scatter.txt") {
			copy_file("$SCAT_NAME_DIR/MT8127_Android_scatter.txt",$COPY_SCATTER_BR_FILES_PATH);
		}
		if (-e "$SCAT_NAME_DIR/partition_size.mk") {
