...
/*Now the max clock num in one module is 6*/
#define MAX_CLK_NUM_OF_ONE_MODULE 6
struct disp_device {
	...
	struct device *dev[DISP_REG_NUM];
	struct device *dev_use_imu;	/* The device use iommu */
...
static inline unsigned long ddp_addr_convert_pa2va(unsigned int pa)
{
	...
	for (i = 0; i < DISP_REG_NUM; i++) {
		if (disp_dev.regs_va[i] == (pa & (~0xfff))) {
			/* printk("ddp_addr_convert_pa2va success: va=0x%lx --> pa=0x%x!\n",
					va,  (ddp_regbase_pa[i]+(va&0xfffl))); */
			return (disp_dev.regs_va[i] + (pa & 0xfff));
		}
	}
	return 0;
}
